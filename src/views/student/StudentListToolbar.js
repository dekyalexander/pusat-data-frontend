...
export default function StudentListToolbar({
  ...
  return (
    <Grid container alignItems="center" style={{ padding: 16 }} spacing={1}>
        ...
        <FormControl variant="outlined" style={{ width: "200px" }}>
          ...
          >
            ...
            {parallelOptions.map((parallel) => {
              return parallel.jurusan == null ? (
                <MenuItem key={parallel.id} value={parallel.id}>
                  {parallel.name}
                </MenuItem>
              ) : (
                <MenuItem key={parallel.id} value={parallel.id}>
                  {parallel.name} {parallel.jurusan.code}
                </MenuItem>
              );
            })}
          </Select>
      ...
      <Grid item>
        <Button onClick={() => getData(1, 25)}>Apply</Button>
      </Grid>
          ...
          <Conditional condition={filters.is_history === 1}>
            <Grid item style={{ marginBottom: 16 }}>
              <FormControl variant="outlined" style={{ minWidth: 200 }}>
                <InputLabel id="tahun_pelajaran-label">
                  Tahun Pelajaran
                </InputLabel>
                <Select
                ...
                >
                  {tahunOptions.map((tahun_pelajaran) => (
                    <MenuItem
                      key={tahun_pelajaran.id}
                      value={tahun_pelajaran.id}
                    >
