...
export default function RoleDetail(props) {
  ...
  const getunitOptions = async () => {
    const response = await axios.get(endpoint.employeeUnit.root);
    if (response && response.data) {
  ...
  const getlevelOptions = async () => {
    ...
    const response = await axios.get(endpoint.parameter.option, {
      params: params,
    });
    if (response && response.data) {
  ...
  }, [dataAccessOptions, unitOptions, levelOptions, roleOptions]);
        ...
        <Fragment>
          ...
          <Grid container style={{ marginBottom: 16 }}>
            <FormControl
              ...
              style={{ minWidth: 120, width: "50%" }}
            >
              <InputLabel id="subtitute_role_id-label">
                Subtitute Role
              </InputLabel>
              <Select
